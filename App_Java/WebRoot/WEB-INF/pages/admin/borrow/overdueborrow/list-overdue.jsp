...
<div class="bjui-pageHeader">
    <form id="pagerForm" data-toggle="ajaxsearch" action="${basePath }admin/overduePaymentInit.do" method="post">
        ...
        <br/>&nbsp;&nbsp;
        	...
        	<div class="pull-right">
               	<a href="exportLateOverduePayment.do?id=1&&userName=${userName }&&borrowWay=${borrowWay }&&borrowState=${borrowState }" class="btn btn-orange" data-toggle="doexport" data-confirm-msg="确定要导出excel吗？">导出excel</a>
            </div>
        </div>
    </form>
</div>
        ...
        <tbody>
        	...
        	<c:forEach items="${pageBean.page }" var="borrow" varStatus="status">
				<tr data-id="65" align="center">
	                ...
	                <td>
                		<c:if test="${borrow.borrowTypeSubId == 2 }">体验标</c:if>
			           	<c:if test="${borrow.borrowTypeSubId == 3 }">多金宝</c:if>
			           	<c:if test="${borrow.borrowTypeSubId == 4 }">普金保</c:if>
			           	<c:if test="${borrow.borrowTypeSubId == 5 }">新手标</c:if>
			           	<c:if test="${borrow.borrowTypeSubId == 6 }">恒金保</c:if>
	                </td>
	                ...
	                <td>
	                	<c:if test="${borrow.repayStatus == 1 }">
	                		<c:if test="${borrow.isWebRepay == 1 }">
	                			<a href="overduePaymentRepaySubmit.do?tabid=${tabid }&&id=${borrow.id }" class="btn btn-green" data-toggle="doajax" data-confirm-msg="确定要还款吗？">还款</a>
	                		</c:if>
	                	</c:if>
                		
                		<a href="repaymentDetail.do?id=${borrow.id}&tabid=repay1${borrow.id }" class="btn btn-green" 
							data-toggle="navtab" data-id="repay1${borrow.id }" data-title="还款详情">查看</a>
