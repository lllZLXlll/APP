...
function expo(){
    ...
    window.location.href="exportAwardzOldInvest.do?borrowName="+borrowName+"&realName="+realName+"&borrowTime="+
    borrowTime+"&awardMoney="+awardMoney+"&faFangStatus="+faFangStatus+"&pageSize="+pageSize+"&pageCurrent="+pageCurrent;
...
<div class="bjui-pageHeader" >
    <form id="pagerForm" data-toggle="ajaxsearch" action="queryAwardInits.do?activity=2" method="post">
        ...
        <div class="bjui-searchBar">
            ...
            <div>&nbsp;&nbsp;
            	借款标题：<input type="text" name="borrowName" value="${borrowName }" id="borrowName" size="20">
	              ...
	              借款期限：&nbsp;
					<select id="borrowTime" name="borrowTime">
						<option value="-1" ${borrowTime==-1 ? 'selected' : '' }>--请选择--</option>
						<option value="1" ${borrowTime==1 ? 'selected' : '' }>三个月以内</option>
						<option value="2" ${borrowTime==2 ? 'selected' : '' }>三个月以上</option>
					</select>
	              奖励金额：
            	...
            	<button type="button" class="btn-default" data-icon="search" onclick="serachRecommendUser()">查询</button>&nbsp;
          		...
          		<div class="pull-right" style="margin-right:4%;">
            		<a href="updateAwardOldInvestByIds.do" data-toggle="doajaxchecked" data-confirm-msg="确定要发放选中项吗？" data-idname="delids" data-group="ids" class="btn btn-blue">发放选中</a>&nbsp;&nbsp;&nbsp;
            	</div>
            </div>
            <br/>
        </div>
    ...
    <table class="table table-bordered table-hover table-striped table-top" data-selected-multi="true">
        <thead>
            <tr >
                ...
                <th align="center">项目期限类型</th>
                <th align="center">奖资金额（元）</th>
					...
					<c:choose>
						<c:when test="${bean.status == 1}">
							<c:if test="${bean.money<=200}">
								<a href="updateAwardOldInvest.do?id=${bean.id}" data-toggle="doajax" data-id="updateAwardOldInvest" data-mask="true" data-confirm-msg="确定要发放奖励吗？" class="btn btn-green">发放</a>
							</c:if>
							<c:if test="${bean.money>200}">
								<a href="updateAwardOldInvest.do?id=${bean.id}" data-toggle="doajax" data-id="updateAwardOldInvest" data-mask="true" data-confirm-msg="请确认大于两百奖励金额的线下已发放过奖励!" class="btn btn-green">发放</a>
							</c:if>
						</c:when>
						<c:otherwise>
							已发放
