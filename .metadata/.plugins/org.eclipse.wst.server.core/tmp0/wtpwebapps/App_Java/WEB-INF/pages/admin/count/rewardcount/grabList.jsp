...
function expo(){
    ...
    window.location.href="exportAwardzGrabInvest.do?borrowName="+borrowName+"&realName="+realName+"&awardType="+
    awardType+"&faFangStatus="+faFangStatus+"&pageSize="+pageSize+"&pageCurrent="+pageCurrent;
...
<div class="bjui-pageHeader" >
    <form id="pagerForm" data-toggle="ajaxsearch" action="queryAwardInits.do?activity=1" method="post">
        ...
        <div class="bjui-searchBar">
            ...
            <div>&nbsp;&nbsp;
	            ...
	            <select id="awardType" name="awardType" data-toggle="selectpicker">
						...
						<option value="1" ${awardType==1 ? 'selected' : '' }>首投</option>
						<option value="2" ${awardType==2 ? 'selected' : '' }>标王</option>
						<option value="3" ${awardType==3 ? 'selected' : '' }>尾投</option>
				</select>&nbsp;
            	发放状态：
            	...
            	<button type="button" class="btn-default" data-icon="search" onclick="serachRecommendUser()">查询</button>&nbsp;
          		...
          		<div class="pull-right" style="margin-right:10%;">
            		<a href="updateAwardGrabInvestByIds.do" data-toggle="doajaxchecked" data-confirm-msg="确定要发放选中项吗？" data-idname="delids" data-group="ids" class="btn btn-blue">发放选中</a>&nbsp;&nbsp;&nbsp;
            	</div>
            </div>
            <br/>
        </div>
        ...
        <thead>
        	...
        	<c:forEach items="${pageBean.page }" var="bean" varStatus="status">
              <tr data-id="65" align="center">
				...
				<td align="center">
					<c:if test="${ bean.awardType==1}">
					 	首投
					</c:if>
					<c:if test="${ bean.awardType==2}">
				 		标王
					</c:if>
					<c:if test="${ bean.awardType==3}">
						尾投
					</c:if>
				</td>
				<td align="center">
					...
					<c:choose>
						<c:when test="${bean.statusStr=='未发放' }">
							<c:if test="${bean.money<200}">
								<a href="updateAwardGrabInvest.do?id=${bean.id}" data-toggle="doajax" data-id="updateAppOperateReportByIdInit" data-mask="true" data-confirm-msg="确定要发放奖励吗？" class="btn btn-green">发放</a>
							</c:if>
							<c:if test="${bean.money>=200}">
								<a href="updateAwardGrabInvest.do?id=${bean.id}" data-toggle="doajax" data-id="updateAppOperateReportByIdInit" data-mask="true" data-confirm-msg="请确认大于两百奖励金额的线下已发放过奖励!" class="btn btn-green">发放</a>
							</c:if>
						</c:when>
						<c:otherwise>
							${bean.statusStr}
