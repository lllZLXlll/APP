...
<div class="bjui-pageHeader">
    ...
    <form id="pagerForm" data-toggle="ajaxsearch" action="loanManageInit.do" method="post">
    	<input type="hidden" name="tabid" value="${tabid}">
	        <input type="hidden" name="pageSize" value="${pageSize}">
	        <input type="hidden" name="pageCurrent" value="${model.pageCurrent}">
    	<div class="bjui-searchBar">
	        
	        <br/>&nbsp;&nbsp;<a data-toggle="navtab" href="borrowInspectionInit.do?tabid=submenu-tab-607" data-id="submenu-tab-607" data-mask="true" data-width="500" data-height="500" class="btn btn-blue">未处理的月检</a>&nbsp;
				...
				<tr data-id="65" align="center">
	                ...
	                <c:choose>
	                	<c:when test="${item.inspectCount>0}">
					            <a href="queryBorrowInspectRecord.do?borrowId=${item.id }&tabid=queryInspect-tab-${item.id }&inspectId=${item.inspectId}" data-toggle="navtab" 
                   				data-id="queryInspect-tab-${item.id }" data-title="${item.borrowTitle }-月检详情" class="btn btn-green">查看月检</a>
					        </c:when>
	                	<c:otherwise>
					          	<a href="saveOrUpdateBorrowMonthInspectInit.do?borrowId=${item.id }&tabid=&inspectId=${item.inspectId}" data-toggle="navtab" 
                   data-id="saveOrUpdateAdminInitDialog" data-mask="true" data-title="${item.borrowTitle }-新增月检" 
