...
function expo(){
    ...
    window.location.href="exportAwardInviteInvest.do?faFangStatus="+faFangStatus+"&realName="+realName+"&awardMoney="+
    awardMoney+"&pageSize="+pageSize+"&pageCurrent="+pageCurrent;
...
<div class="bjui-pageHeader" >
    <form id="pagerForm" data-toggle="ajaxsearch" action="queryAwardInits.do?activity=3" method="post">
        ...
        <div class="bjui-searchBar">
            ...
            <br/>
            <div>&nbsp;&nbsp;
	              推荐人：<input type="text" name="realName" value="${realName }" id="realName" size="20">
	              奖励金额：
            	...
            	<button type="button" class="btn-default" data-icon="search" onclick="serachRecommendUser()">查询</button>&nbsp;
          		...
          		<div class="pull-right" style="margin-right:4%;">
            		<a href="updateAwardInviteInvestByIds.do" data-toggle="doajaxchecked" data-confirm-msg="确定要发放选中项吗？" data-idname="delids" data-group="ids" class="btn btn-blue">发放选中</a>&nbsp;&nbsp;&nbsp;
            	</div>
            </div>
            <br/>
        </div>
					...
					<c:choose>
						<c:when test="${bean.statusStr=='未发放'}">
							<c:if test="${bean.money<=200}">
								<a href="updateAwardInviteInvest.do?id=${bean.id}" data-toggle="doajax" data-id="updateAwardInviteInvest" data-mask="true" data-confirm-msg="确定要发放奖励吗？" class="btn btn-green">发放</a>
							</c:if>
							<c:if test="${bean.money>200}">
								<a href="updateAwardInviteInvest.do?id=${bean.id}" data-toggle="doajax" data-id="updateAwardInviteInvest" data-mask="true" data-confirm-msg="请确认大于两百奖励金额的线下已发放过奖励!" class="btn btn-green">发放</a>
							</c:if>
						</c:when>
						<c:otherwise>
							${bean.statusStr}
