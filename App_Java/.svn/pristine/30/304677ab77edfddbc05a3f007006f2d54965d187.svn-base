        ...
        <tbody>
        	...
        	<c:forEach items="${pageBean.page }" var="item" varStatus="status">
				<tr data-id="65" align="center">
	                ...
	                <td>
						<c:if test="${item.result==0 }"><span style="color: red">失败</span>    </c:if>
						<c:if test="${item.result==1 }"><span style="color: green">成功</span></c:if>
						<c:if test="${item.result==2 }"><span style="color: orange">审核中</span></c:if>
					</td>
            	</tr>
            </c:forEach>
	                	</c:otherwise>
